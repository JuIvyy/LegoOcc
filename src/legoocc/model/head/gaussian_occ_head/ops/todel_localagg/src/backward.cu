...
__global__ void renderCUDA(
    int P,
    int total_channels,                             // total channels
    ...
    int base_channel                                  // offset for current channel group
)
{
    ...
    for (int i = start; i < end; ++i) {
        ...
        if (pts_idx >= 0) {
            ...
            for (int ch = 0; ch < CHANNELS; ++ch) {
                int global_ch = base_channel + ch;
                if (global_ch < total_channels) {
                    float curr_out_grad = power * out_grad[pts_idx * total_channels + global_ch];
    ...
    // Write gradients for this group of channels
    ...
    while (current_channel < C) {
        ...
        if (left_channels >= 128) {
            renderCUDA<128><<<blocks_per_grid, threads_per_block>>>(
                P, C, offsets, point_list_keys_unsorted, voxel2pts, pts,
                means3D, cov3D, opacity, semantic, out_grad,
                means3D_grad, opacity_grad, semantics_grad, cov3D_grad,
                current_channel
            );
            current_channel += 128;
        }
