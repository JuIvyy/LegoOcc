...
__global__ void renderCUDA(
    ...
    const float* __restrict__ logits,       // [N, C]  (forward conditional semantics)
    const float* __restrict__ bin_logits,   // [N]     (o = 1 - T)
    const float* __restrict__ density,      // [N]     (z = sum α_i)
    const float* __restrict__ probability,  // [N]     (T = prod (1 - α_i))
    const float* __restrict__ logits_grad,  // [N, C]  (∂L/∂y_c)
    const float* __restrict__ bin_logits_grad, // [N]   (∂L/∂o)
    ...
    int gi = blockIdx.x * blockDim.x + threadIdx.x; // gaussian index
    if (gi >= P) return;

    uint32_t start = (gi == 0) ? 0 : offsets[gi - 1];
    uint32_t end   = offsets[gi];

    float3 m  = {means3D[3*gi+0], means3D[3*gi+1], means3D[3*gi+2]};
    float3 c1 = {cov3D[6*gi+0],   cov3D[6*gi+1],   cov3D[6*gi+2]};
    float3 c2 = {cov3D[6*gi+3],   cov3D[6*gi+4],   cov3D[6*gi+5]};
    float  a  = opas[gi];

    float svec[CHANNELS] = {0.f};
    #pragma unroll
    for (int ch = 0; ch < CHANNELS; ++ch) {
        int gch = base_channel + ch;
        if (gch < total_channels) svec[ch] = semantic[gi * total_channels + gch];
    }

    float g_m[3]   = {0.f,0.f,0.f};
    float g_cov[6] = {0.f,0.f,0.f,0.f,0.f,0.f};
    float g_a      = 0.f;
    float g_s[CHANNELS] = {0.f};

    const float eps = 1e-9f;

    for (uint32_t it = start; it < end; ++it) {
        int vx = (int)point_list_keys_unsorted[it];
        int p  = voxel2pts[vx];   // pixel/voxel index on the other side
        if (p < 0) continue;

        float3 d = { m.x - pts[3*p+0], m.y - pts[3*p+1], m.z - pts[3*p+2] };
        float quad  = c1.x*d.x*d.x + c1.y*d.y*d.y + c1.z*d.z*d.z;
        float cross = c2.x*d.x*d.y + c2.y*d.y*d.z + c2.z*d.x*d.z;
        float phi   = __expf(-0.5f*quad - cross);

        // read forward aux
        float z = fmaxf(density[p], eps);      // sum α_i
        float T = fmaxf(probability[p], eps);  // prod(1-α_i)
        float o = bin_logits[p];               // 1 - T

        // reconstruct alpha_i for grad
        float alpha_i = a * phi;
        alpha_i = fminf(fmaxf(alpha_i, 0.f), 1.f - 1e-6f);

        // ----- semantic path: dL/dα_i and dL/ds_i -----
        float g_dalpha_sem = 0.f;
        #pragma unroll
        for (int ch = 0; ch < CHANNELS; ++ch) {
            int gch = base_channel + ch;
            if (gch < total_channels) {
                float y_c   = logits[p * total_channels + gch];
                float dL_dy = logits_grad[p * total_channels + gch];
                // ∂y_c/∂α_i = (s_i[c] - y_c) / z
                g_dalpha_sem += dL_dy * (svec[ch] - y_c) / z;
                // ∂y_c/∂s_i[c] = α_i / z
                g_s[ch] += dL_dy * (alpha_i / z);
            }
        }

        // ----- occupancy path: o = 1 - T,  ∂o/∂α_i = T / (1 - α_i) -----
        float dL_do = bin_logits_grad[p];
        float g_dalpha_occ = dL_do * (T / fmaxf(1.f - alpha_i, 1e-6f));

        // ----- optional: density supervision z -----
        float g_dalpha_den = density_grad[p];  // since ∂z/∂α_i = 1

        float g_dalpha = g_dalpha_sem + g_dalpha_occ + g_dalpha_den;

        // chain to opacity and power
        g_a += g_dalpha * phi;
        float dL_dphi = g_dalpha * a;

        // phi = exp(arg) ⇒ ∂phi/∂arg = phi
        float g_arg = dL_dphi * phi;
    ...
    atomicAdd(&means3D_grad[3*gi+0], g_m[0]);
    atomicAdd(&means3D_grad[3*gi+1], g_m[1]);
    atomicAdd(&means3D_grad[3*gi+2], g_m[2]);

    atomicAdd(&opas_grad[gi], g_a);

    #pragma unroll
    for (int ch = 0; ch < CHANNELS; ++ch) {
        int gch = base_channel + ch;
        if (gch < total_channels)
            atomicAdd(&semantics_grad[gi * total_channels + gch], g_s[ch]);
    }

    #pragma unroll
    for (int k = 0; k < 6; ++k)
        atomicAdd(&cov3D_grad[6*gi + k], g_cov[k]);
}
