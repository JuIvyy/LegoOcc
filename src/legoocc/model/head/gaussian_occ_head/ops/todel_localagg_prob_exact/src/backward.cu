...
// Strict physically-faithful HGS backward
// forward used: z = sum_i (o_tilde_i * phi_i),  alpha = 1 - exp(-z)
// where o_tilde = -log(1 - o), o in (0,1) after clamping
...
__global__ void renderCUDA(
    ...
    const float* __restrict__ cov3D,        // [P, 6] (c1: diag xyz, c2: off-diag xy,yz,xz)
    const float* __restrict__ opas,         // [P]   original GS opacity o in (0,1)
    const float* __restrict__ semantic,     // [P, total_channels]

    // forward outputs / caches
    const float* __restrict__ logits,       // [N, total_channels] (p(c|occ))
    const float* __restrict__ bin_logits,   // [N]   alpha = 1 - exp(-z)
    const float* __restrict__ density,      // [N]   z
    const float* __restrict__ probability,  // [N]   z (kept for compat)

    // upstream grads
    const float* __restrict__ logits_grad,  // [N, total_channels]  dL/d(logits_out)
    const float* __restrict__ bin_logits_grad, // [N] dL/d(alpha)
    const float* __restrict__ density_grad,    // [N] dL/d(z) optional

    // grads to write
    float* __restrict__ means3D_grad,       // [P, 3]
    float* __restrict__ opas_grad,          // [P]
    float* __restrict__ semantics_grad,     // [P, total_channels]
    float* __restrict__ cov3D_grad,         // [P, 6]
    int base_channel
)
{
    // numerical guards
    const float EPS  = 1e-6f;
    const float OMIN = 1e-6f;
    const float OMAX = 1.f - 1e-6f;
    ...
    const float o_raw= opas[idx];
    const float o    = fminf(fmaxf(o_raw, OMIN), OMAX);
    const float inv1 = fmaxf(1.f - o, EPS);
    const float o_tilde = -__logf(inv1);       // = -log(1 - o)
    const float d_otilde_do = 1.f / inv1;      // d(-log(1-o))/do

    // load this gaussian's semantic slice
    ...
    // accumulators for this gaussian's gradients
    float g_m[3]    = {0.f, 0.f, 0.f};
    float g_cov[6]  = {0.f, 0.f, 0.f, 0.f, 0.f, 0.f};
    float g_o_tilde = 0.f;                    // grad wrt o_tilde (then chain to o)
    float g_sem[CHANNELS] = {0.f};

    // iterate pixels/voxels this gaussian influences
    for (uint32_t it = start; it < end; ++it) {
        const int voxel_idx = (int)point_list_keys_unsorted[it];
        const int pidx = voxel2pts[voxel_idx];
        if (pidx < 0) continue;

        // read forward caches at pixel p
        const float z   = probability[pidx];          // == density[pidx]
        const float occ = bin_logits[pidx];           // alpha = 1 - exp(-z)
        if (z <= EPS) continue;                       // no grad when no intensity

        // geometry -> kernel value φ(x)
        ...
        // h_i = o_tilde * phi
        const float h_i = o_tilde * phi;

        // ---------- semantic path ----------
        // logits y_c = sum_j h_j * sem_j[c] / z  (conditional semantics)
        // dL/dh_i_sem = sum_c dL/dy_c * (sem_i[c] - y_c) / z
        float g_h_sem = 0.f;
        #pragma unroll
        for (int ch = 0; ch < CHANNELS; ++ch) {
            int gch = base_channel + ch;
            if (gch < total_channels) {
                float y_c   = logits[pidx * total_channels + gch];
                float dL_dy = logits_grad[pidx * total_channels + gch];
                g_h_sem    += dL_dy * (sem[ch] - y_c) / z;  // ∂y/∂h_i
                g_sem[ch]  += dL_dy * (h_i / z);            // ∂y/∂sem_i[c]
            }
        }

        // ---------- occupancy & density path ----------
        // alpha = 1 - exp(-z)  =>  ∂alpha/∂z = exp(-z) = 1 - alpha
        float dL_dz = 0.f;
        dL_dz += bin_logits_grad[pidx] * (1.f - occ);
        dL_dz += density_grad[pidx];   // optional supervision on z

        // total gradient wrt h_i  (since z = sum_j h_j)
        const float g_h_total = g_h_sem + dL_dz;

        // split to o_tilde and phi:
        // h_i = o_tilde * phi  =>  ∂h/∂o_tilde = phi,  ∂h/∂phi = o_tilde
        g_o_tilde        += g_h_total * phi;
        const float dL_dphi = g_h_total * o_tilde;

        // ---------- chain to means & cov via phi = exp(arg) ----------
        // arg = -0.5 * (c1.x dx^2 + c1.y dy^2 + c1.z dz^2) - (c2.x dx dy + c2.y dy dz + c2.z dx dz)
        // ∂phi/∂arg = phi
        const float g_arg = dL_dphi * phi;
        ...
        // covariances
        g_cov[0] += g_arg * (-0.5f * d.x * d.x);                   // c1.x
        g_cov[1] += g_arg * (-0.5f * d.y * d.y);                   // c1.y
        g_cov[2] += g_arg * (-0.5f * d.z * d.z);                   // c1.z
        g_cov[3] += g_arg * (- d.x * d.y);                         // c2.x (xy)
        g_cov[4] += g_arg * (- d.y * d.z);                         // c2.y (yz)
        g_cov[5] += g_arg * (- d.x * d.z);                         // c2.z (xz)
    }

    // chain to original opacity parameter o:  dL/do = dL/d(o_tilde) * d(o_tilde)/do
    const float g_o = g_o_tilde * d_otilde_do;
