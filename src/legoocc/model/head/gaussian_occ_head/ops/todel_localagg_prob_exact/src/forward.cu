...
// Strict, physically-faithful HGS forward:
//   1) map per-Gaussian opacity o -> optical-depth amplitude o_tilde = -log(1 - o)
//   2) h_i(x) = o_tilde * phi_i(x);  z(x) = sum_i h_i(x);  alpha(x) = 1 - exp(-z)
...
__global__ void renderCUDA(
    ...
    const float* __restrict__ opas,        // 3DGS per-Gaussian "opacity" o_i in (0,1)
    const float* __restrict__ semantic,
    float* __restrict__ out_logits,        // [N, total_channels] : p(c | occ)
    float* __restrict__ out_bin_logits,    // [N] : alpha(x) = 1 - exp(-z)
    float* __restrict__ out_density,       // [N] : z(x) (optical depth / hazard)
    float* __restrict__ out_probability,   // [N] : kept as z for backward-compat
    int base_channel
)
{
    // ---- constants for numerical safety
    const float EPS  = 1e-6f;      // for division/exp safety
    const float OMIN = 1e-6f;      // clamp lower for opacity
    const float OMAX = 1.f - 1e-6f;// clamp upper for opacity
    ...
    float z = 0.f;               // optical depth sum: z = sum_i (o_tilde_i * phi_i(x))

    for (int i = range.x; i < range.y; ++i) {
        const int gs_idx = point_list[i];

        // ---- 3D Gaussian kernel value: phi_i(x)
        ...
        // ---- strict mapping: opacity param o -> optical-depth amplitude o_tilde = -log(1 - o)
        float o = fminf(fmaxf(opas[gs_idx], OMIN), OMAX);
        float o_tilde = -__logf(1.f - o);  // optical-depth per unit-kernel

        // ---- local optical depth contribution
        float h_i = o_tilde * phi;

        // accumulate optical depth and semantic numerator
        z += h_i;
        #pragma unroll
    ...
    // conditional semantics p(c|occ): intensity-normalized
    if (z > EPS) {
        #pragma unroll
        for (int ch = 0; ch < CHANNELS; ++ch) {
            int gch = base_channel + ch;
    ...
    if (base_channel == 0) {
        // Beer–Lambert: alpha(x) = 1 - exp(-z)
        float occ = 1.f - __expf(-z);
        out_bin_logits[idx]  = occ;  // in [0,1]
        out_density[idx]     = z;    // store optical depth
        out_probability[idx] = z;    // kept as z for backward use
    }
}
