...
def global_lovasz_softmax(probas, labels, classes='present', per_image=False, ignore=None):
    """
    ...
    if per_image:
        ...
        loss = lovasz_softmax_flat(*global_flatten_probas(probas, labels, ignore), classes=classes)
    return loss

def lovasz_softmax(probas, labels, classes='present', per_image=False, ignore=None, fov_mask=None):
    """
    Multi-class Lovasz-Softmax loss
      probas: [B, C, H, W] Variable, class probabilities at each prediction (between 0 and 1).
              Interpreted as binary (sigmoid) output with outputs of size [B, H, W].
    ...
    if per_image:
        ...
        loss = lovasz_softmax_flat(*flatten_probas(probas, labels, ignore, fov_mask), classes=classes)
    return loss
...
def global_flatten_probas(probas, labels, ignore=None):
    """
    ...
    if ignore is None:
        # vprobas = probas[fov_mask.nonzero().squeeze()]
        # vlabels = labels[fov_mask.nonzero().squeeze()]
        return probas, labels
    valid = (labels != ignore)
...
def flatten_probas(probas, labels, ignore=None, fov_mask=None):
    """
    Flattens predictions in the batch
    [1, 13, 60, 60, 36]
    [1, 60, 60, 36]
