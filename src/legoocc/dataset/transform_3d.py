...
class PhotoMetricDistortionMultiViewImage:
    ...
    def __call__(self, results):
        ...
        for img in imgs:
            ...
            if self.use_swap_channel and random.randint(2):
...
class ImageAug3D:
    def __init__(
        self, final_dim, resize_lim, is_train
    ):
    ...
    def img_transform(
        self, img, rotation, translation, resize, resize_dims, crop
    ):
        # adjust image
        img = mmcv.imresize(img=img, size=resize_dims[::-1])    # img_shape = resize_dims
        img = img[crop[1]:crop[3], crop[0]:crop[2]]

        # post-homography transformation
        rotation *= resize
        translation -= torch.Tensor(crop[:2])                   # u, v --> W, H
    ...
    def __call__(self, data):
        ...
        for i, img in enumerate(imgs):
            ...
            resize, resize_dims, crop = self.sample_augmentation(data)
